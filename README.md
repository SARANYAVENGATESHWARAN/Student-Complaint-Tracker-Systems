...
The application follows a clear complaint workflow: Submit → Allocate → Update → Report. All updates are reflected in real time across dashboards, giving users a clear view of complaint progress. The system also supports downloading reports in PDF and CSV formats, making it useful for record keeping and analysis.

This project is built completely on the frontend without a backend or database. Complaint data is managed using JSON objects and local state, making it lightweight and easy to understand. The project demonstrates core frontend development skills such as component-based design, state management, routing, and data visualization.
